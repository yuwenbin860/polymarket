...
- **典型例子**:
  ...
  - "BTC突破$150k" → "BTC突破$100k" (上涨阈值：更高阈值蕴含更低阈值)
  - "Stock跌到$50" → "Stock跌到$100" (下跌阈值：更低阈值蕴含更高阈值，跌到更低价必先跌过更高价)
  - "在2025年1月前发生" → "在2025年6月前发生" (更早时间被更晚时间包含)
- **特别注意 - 上涨vs下跌阈值方向相反**:
  - 上涨(above/hit/突破): 更高阈值 → 更低阈值 (如 $150k → $100k)
  - 下跌(dip/below/跌到): 更低阈值 → 更高阈值 (如 $50 → $100)

### 2. IMPLIES_BA (B蕴含A)
- **定义**: 如果B发生，A必然发生。与IMPLIES_AB相反。
- **数学约束**: P(A) ≥ P(B)
- **典型例子**:
  - "BTC突破$100k" → "BTC突破$150k" (B→A，更低阈值不蕴含更高阈值)
  - "Stock跌到$100" → "Stock跌到$50" (B→A，更高阈值不蕴含更低阈值)
- **警告**: 不要将上涨阈值的蕴含方向与下跌阈值混淆！
